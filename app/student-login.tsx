...
export default function StudentLogin() {
  ...
  useEffect(() => {
    authService.getRememberedCredential().then((cred) => {
      if (cred && cred.role === 'student' && cred.username) {
        setStudentId(cred.username);
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const result = await authService.login('student', studentId.trim(), password, rememberMe);
      ...
      <View style={styles.contentContainer}>
        ...
        <View style={styles.headerArea}>
          <View style={styles.iconBox}>
            <IconSymbol size={32} name="person.crop.circle.fill" color={APP_COLORS.primaryWarm} />
          </View>
          <View style={styles.portalBadge}>
            <Text style={styles.portalBadgeText}>STUDENT PORTAL</Text>
          </View>
